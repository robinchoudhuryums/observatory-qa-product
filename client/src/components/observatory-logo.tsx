/**
 * Observatory SVG logo component with inline SVG for full CSS control.
 * Supports: dark/light mode (via currentColor), custom colors, icon-only variant,
 * hover transitions, loading animation, and animated entrance.
 */
...
interface ObservatoryLogoProps {
  ...
  height?: number;
  /** Override fill color (defaults to currentColor for theme auto-switching) */
  color?: string;
  /** Show pulsing animation (for loading states) */
  animated?: boolean;
  /** Enable hover color transition */
  hoverable?: boolean;
  /** Hover color (defaults to brand teal) */
  hoverColor?: string;
  className?: string;
}

/** Shared owl icon SVG paths */
const OWL_PATHS = (
  <>
...
function OwlIcon({ size = 36, color, animated }: { size?: number; color?: string; animated?: boolean }) {
  return (
    ...
    >
      <g transform="translate(3, -12) scale(0.78)">
        {OWL_PATHS}
      </g>
    </svg>
  );
...
export function ObservatoryLogo({
  variant = "full",
  height = 36,
  color,
  animated = false,
  ...
  hoverColor = "#14b8a6",
  className,
}: ObservatoryLogoProps) {
  const [hovered, setHovered] = useState(false);
  const resolvedColor = hovered && hoverable ? hoverColor : color;

  const wrapperProps = hoverable
    ? {
        onMouseEnter: () => setHovered(true),
        onMouseLeave: () => setHovered(false),
        style: { transition: "color 200ms ease, fill 200ms ease" } as React.CSSProperties,
      }
    : {};
  ...
  if (variant === "icon") {
    return (
      <span
        ...
        {...wrapperProps}
      >
        <OwlIcon size={height} color={resolvedColor} animated={animated} />
      </span>
    );
  }

  // Full logo: icon + text, aspect ratio ~3.73:1 from original viewBox
  const width = Math.round(height * 3.73);
  ...
  return (
    ...
    >
      <svg
        ...
        aria-hidden="true"
        className={animated ? "animate-pulse" : undefined}
        style={hoverable ? { transition: "fill 200ms ease" } : undefined}
      >
        {/* Owl icon */}
        <g transform="scale(1.4046) translate(-27,-20)">
          {OWL_PATHS}
        </g>
        {/* "Observatory" text */}
        <g transform="translate(82, 3) scale(2.641)">
          <path d={TEXT_PATH} />
        </g>
      </svg>
    </span>
