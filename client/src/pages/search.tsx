...
export default function SearchPage() {
  ...
  const { data: allCalls, isLoading: isLoadingCalls } = useQuery<CallWithDetails[]>({
    queryKey: ["/api/calls", { employee: employeeFilter, sentiment: sentimentFilter, status: statusFilter }],
            ...
            {isLoading ? (
              ...
              <div className="space-y-4">
                {displayCalls.map((call, index) => {
                  ...
                  try {
                    return (
                      <Card key={call?.id || index} className="hover:shadow-md transition-shadow">
                    ...
                    console.error("THE FOLLOWING CALL OBJECT CAUSED THE CRASH:");
                    console.error(call);
                    console.error("THE SPECIFIC ERROR WAS:");
                    console.error(error);
                    console.error("--- END OF CRASH REPORT ---");
                    return (<div key={index} className="p-4 border border-red-500 bg-red-50 rounded-lg">Error rendering this item. Please check the browser console for the problematic data object.</div>);
                  }
